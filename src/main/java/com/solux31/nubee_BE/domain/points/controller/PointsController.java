...
public class PointsController {
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "인증 실패 (토큰 없음/만료)"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "500", description = "서버 오류")
    })
    @GetMapping
    public ResponseEntity<ApiResponse<PointsResDTO.PointInfo>> getPoints(
            @AuthenticationPrincipal AuthUser authUser
    ) {
        PointsResDTO.PointInfo result = pointsService.getPoints(authUser.getUserId());
